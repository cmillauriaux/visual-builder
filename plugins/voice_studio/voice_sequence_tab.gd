...
func _on_generate_all_pressed() -> void:
	...
	# Construire la queue de génération (un par dialogue, séquentiellement)
	...
	_set_status(tr("Génération en cours... 1/%d") % _pending_generation.size())
	_generate_all_btn.disabled = true
...
func _on_generation_completed(mp3_bytes: PackedByteArray, request_id: String) -> void:
	...
	if not _pending_generation.is_empty():
		...
		if remaining > 0:
			_set_status(tr("Génération en cours... %d/%d") % [_pending_generation.size(), _pending_generation.size() + 1])
			_process_next_in_queue()
		else:
			_generate_all_btn.disabled = false
			_set_status(tr("Toutes les voix ont été générées !"))
