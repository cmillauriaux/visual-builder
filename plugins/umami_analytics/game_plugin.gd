...
## Plugin d'analytics Umami : encapsule le service Umami dans un plugin in-game.
## Lit la configuration depuis story.plugin_settings["umami_analytics"]
## et envoie les événements de télémétrie aux hooks du jeu.

const UmamiAnalyticsServiceScript = preload("res://plugins/umami_analytics/umami_analytics_service.gd")
...
func get_plugin_description() -> String:
	return "Envoie les événements de jeu à Umami Analytics"
...
func _setup_service() -> void:
	...
	_service = Node.new()
	_service.set_script(UmamiAnalyticsServiceScript)
	_service.name = "UmamiAnalytics"
...
func _create_options_control(settings: RefCounted) -> Control:
	...
	title_label.text = "Umami Analytics"
...
## Configuration éditeur : champs Website ID, Umami URL et Enabled pour Umami.
...
func _create_editor_config(current_settings) -> Control:
	...
	url_lbl.text = "Umami URL"
	vbox.add_child(url_lbl)

	var url_edit := LineEdit.new()
	url_edit.name = "UmamiUrlEdit"
	url_edit.placeholder_text = "https://cloud.umami.is"
	url_edit.text = ps.get("umami_url", "")
	vbox.add_child(url_edit)

	var site_lbl := Label.new()
	site_lbl.text = "Website ID (UUID)"
	vbox.add_child(site_lbl)

	var site_edit := LineEdit.new()
	site_edit.name = "UmamiWebsiteIdEdit"
	site_edit.placeholder_text = "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
	site_edit.text = ps.get("website_id", "")
	vbox.add_child(site_edit)

	var enabled_check := CheckButton.new()
	enabled_check.name = "UmamiEnabledCheck"
	enabled_check.text = "Activer le tracking Umami"
	...
	vbox.set_meta("_url_edit", url_edit)
	vbox.set_meta("_site_edit", site_edit)
...
func read_editor_config(control: Control) -> Dictionary:
	if control == null:
		return {}
	var url_edit = control.get_meta("_url_edit", null)
	var site_edit = control.get_meta("_site_edit", null)
	var enabled_check = control.get_meta("_enabled_check", null)
	return {
		"umami_url": url_edit.text if url_edit else "",
		"website_id": site_edit.text if site_edit else "",
