...
## Onglet Enhance : améliore la qualité d'une image sans changer ses dimensions.
	...
	_result_preview.gui_input.connect(func(event: InputEvent):
		if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
			if _result_preview.texture:
				_show_preview_fn.call(_result_preview.texture, "Résultat Enhance")
	)
...
func _on_generate_pressed() -> void:
	...
	_client.generate_upscale_enhance(
		config,
		_source_image_path,
		ComfyUIClient.WorkflowType.ENHANCE,
		_prompt_input.text,
		1.0,
...
func _on_generation_completed(image: Image) -> void:
	...
	_show_success("Am\u00e9lioration termin\u00e9e !")
	var timestamp = str(Time.get_unix_time_from_system()).replace(".", "_")
	_name_input.text = "ai_enhance_" + timestamp
...
func _on_save_pressed() -> void:
	...
	if img_name == "":
		var timestamp = str(Time.get_unix_time_from_system()).replace(".", "_")
		img_name = "ai_enhance_" + timestamp
