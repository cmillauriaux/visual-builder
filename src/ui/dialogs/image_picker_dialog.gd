...
func _list_gallery_images() -> Array:
	...
	if _mode == Mode.FOREGROUND and _story_base_path != "":
		...
		# Dédupliquer par nom de fichier (garder foregrounds/ si doublon)
		var existing_filenames := {}
		for p in images:
			existing_filenames[p.get_file()] = true
		for p in anim_images:
			if not existing_filenames.has(p.get_file()):
				images.append(p)
	return images
